...
void demote_buffers(long long *p_n, double *p_src, double *p_dst)
{
  long long n = *p_n;
...
double cpu_scale(long long *p_n, double *p_src, double scalar, double *p_dst)
{
...
double cpu_omp_scale(long long *p_n, double *p_src, double scalar, double *p_dst)
{
  long long n = *p_n;
...
int check_scale_results(long long *p_n, double *p_exp, double *p_obs)
{
  for (long long i=0; i<*p_n; i++) {
    if (p_exp[i] != p_obs[i]) {
...
double dsa_omp_scale(long long *p_n, double *p_src, double scalar, double *p_dst,
                     double *p_tmp, dml_job_t **p_dml_jobs)
{
  long long n = *p_n;
...
#pragma omp parallel
  {
    ...
    cpu_scale(&tmp_elems, p_t_src, scalar, p_t_dst);

    double t_start, t_dsa_elapsed=0., t_cpu_elapsed=0.;
    int j;
    for (j=QUEUE_DEPTH; j<(num_blks-QUEUE_DEPTH); j++) {
      /* t_start = omp_get_wtime(); */
      dml_wait_job(p_t_dml_jobs[j%QUEUE_DEPTH]);
      /* t_dsa_elapsed += omp_get_wtime() - t_start; */

      /* t_start = omp_get_wtime(); */
      cpu_scale(&blk_elems, p_t_tmp+((j%QUEUE_DEPTH)*blk_elems), scalar, p_t_dst+(j*blk_elems));
    ...
    for (int i=j; i<(j+QUEUE_DEPTH); i++) {
      dml_wait_job(p_t_dml_jobs[i%QUEUE_DEPTH]);
      cpu_scale(&blk_elems, p_t_tmp+((i%QUEUE_DEPTH)*blk_elems), scalar, p_t_dst+(i*blk_elems));
    }

    if (tail) {
      cpu_scale(&tail, p_t_src+(num_blks*blk_elems), scalar, p_t_dst+(num_blks*blk_elems));
    }
...
{
  double *p_src = NULL, *p_dst = NULL, *p_dst_ref = NULL, *p_t_elapsed = NULL;
  double t_start, scalar = 3.;
...
#pragma omp parallel for
  ...
  cpu_scale(&num_elems, p_src, scalar, p_dst_ref);
  ...
  for (int i=0; i<NTRIALS; i++) {
    ...
    demote_buffers(&num_elems, p_src, p_dst);
#endif
    t_start = dsecnd();

#if defined (USE_CPU)
    cpu_omp_scale(&num_elems, p_src, scalar, p_dst);
#elif defined (USE_DSA)
    dsa_omp_scale(&num_elems, p_src, scalar, p_dst, p_tmp, p_dml_jobs);
#endif

    p_t_elapsed[i] = dsecnd() - t_start;
    printf ("Iter[%d]: N = %ld, Size(MB) = %.2f, Time (us) = %.2f, Bandwidth(GB/s) = %.4f\n",
            i, num_elems, size_mB, p_t_elapsed[i]*1.e6, (2.*size_gB)/(p_t_elapsed[i]));

    if (check_scale_results(&num_elems, p_dst_ref, p_dst)) {
  ...
  printf ("N = %ld, Size(MB) = %.2f, Best Time(us) = %.2f, Best Bandwidth(GB/s) = %.4f\n",
          num_elems, size_mB, t_best*1.e6, (2.*size_gB)/(t_best));
